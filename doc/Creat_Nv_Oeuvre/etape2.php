...
// valid2 permet de vérifier la validité des informations pour la création d'une oeuvre
$valid2 = 1;
...
{
	if($valid2 && !(checkTitreAuteur($_POST['titre'], $_POST['auteur'])))
	{
		$valid2 = 0;
		$erreurAuteur = 1;
	}
	else if($valid2 && !(checkPartitionStyle($_POST['partition'], $_POST['style'])))
	{
		$valid2 = 0;
		$erreurPartition = 1;
	}
	else if ($valid2 && !(checkDuree($_POST["duree"])))
	{
		$valid2 = 0;
		$erreurDuree = 1;
	}
...
<html>
	...
	<body>
		...
		{
			if(isset($erreurAuteur))
			{
				$erreur = 'Erreur avec le champs "Auteur" ou "Titre" : format ou valeur incorrect.';
			}			
			else if(isset($erreurPartition))
			{
				$erreur = 'Erreur avec le champs "Partition" ou "Style" : format ou valeur incorrect.';
			}
			else if(isset($erreurDuree))
			{
				$erreur = 'Erreur avec le champs "Duree" : format ou valeur incorrect.';
			}
			if(isset($erreurAuteur) || isset($erreurPartition) || isset($erreurDuree))
			{
				echo '<p><span style="color:red;">'.$erreur.'</span><br />'.
						'<a href="index.php">Retour à l\'étape n°1.</a></p>';
			}
